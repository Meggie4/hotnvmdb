...
/* Job queue */
typedef struct jobqueue{//等待工作队列
    pthread_mutex_t rwmutex;   //队列的互斥锁          /* used for queue r/w access */
    job  *front;     //工作队列的头节点                    /* pointer to front of queue */
    job  *rear;     //工作队列的尾部节点                     /* pointer to rear  of queue */
    bsem *has_jobs;   //信号量                   /* flag as binary semaphore  */
    volatile int   len; //队列长度          /* number of jobs in queue   */
} jobqueue;


/* Thread */
typedef struct thread{//线程的封装 
    int       id;           //线程id       /* friendly id               */
    pthread_t pthread;     //线程              /* pointer to actual thread  */
    struct thpool_* thpool_p; //线程属于的线程池           /* access to thpool          */
} thread;
...
/* Threadpool */
typedef struct thpool_{//线程池 
    thread**   threads;     //所包含的线程数组             /* pointer to threads        */
    volatile int num_threads_alive;  //当前存活的线程数目    /* threads currently alive   */
    volatile int num_threads_working; //当前正在工作的线程数目   /* threads currently working */
    pthread_mutex_t  thcount_lock;  //线程锁     /* used for thread count etc */
    jobqueue*  jobqueue_p;  //工作队列             /* pointer to the job queue  */
    int firstime;
} thpool_;
...
struct thpool_* thpool_init(int num_threads){
    ...
    if ( num_threads < 0){//线程数目
        num_threads = 0;
    }

    /* Make new thread pool */
    thpool_* thpool_p;
    thpool_p = (struct thpool_*)malloc(sizeof(struct thpool_));//分配的线程池对象
    if (thpool_p == NULL){
    ...
    if (jobqueue_init(thpool_p) == -1){//初始化工作等待队列 
    ...
    thpool_p->threads = (struct thread**)malloc(num_threads * sizeof(struct thread*));//初始啊胡
    ...
    pthread_mutex_init(&(thpool_p->thcount_lock), NULL);//线程锁 
    ...
    for (n=0; n<num_threads; n++){
        ...
        thread_init(thpool_p, &thpool_p->threads[n], numthreads);//初始化各个线程,以及其在线程组的id号:0, 1...
        if (THPOOL_DEBUG)
            printf("THPOOL_DEBUG: Created thread %d in pool \n", n);
        numthreads++;//已经初始化的线程数目
    }

    /* Wait for threads to initialize */
    while (thpool_p->num_threads_alive != num_threads) {}

    g_newjob=(struct job*)malloc(sizeof(struct job));//当前的工作
    if (g_newjob==NULL){
...
int thpool_add_work(thpool_* thpool_p, void *(*function_p)(void*), void* arg_p){
    ...
    g_newjob->function=function_p;//函数体和参数
    g_newjob->arg=arg_p;
    thpool_p->jobqueue_p->len++;//工作队列的中的长度 
...
static void thread_init (thpool_* thpool_p, struct thread** thread_p, int id){
    ...
    *thread_p = (struct thread*)malloc(sizeof(struct thread));//创建线程 
    if (thread_p == NULL){
        fprintf(stderr, "thpool_init(): Could not allocate memory for thread\n");
        exit(1);
    }

    (*thread_p)->thpool_p = thpool_p;//线程所在的线程池
    (*thread_p)->id       = id;//线程在线程组中的id号
    thpool_p->firstime = 0;//线程

    pthread_create(&(*thread_p)->pthread, NULL, thread_do, (*thread_p));//创建线程，线程函数，线程参数
...
static void* thread_do(void *arg){
    ...
    thpool_* thpool_p = thread_p->thpool_p;//线程池 
    int thread_id = thread_p->id;//线程在线程数组中的id

    /* Mark thread as alive (initialized) */
    pthread_mutex_lock(&thpool_p->thcount_lock);//互斥锁 
    thpool_p->num_threads_alive += 1;//当前alive的线程数目
    pthread_mutex_unlock(&thpool_p->thcount_lock);

    while(threads_keepalive){//线程一直处于运转

        if(!thpool_p->firstime){//第一个工作
          while(thpool_p->jobqueue_p == NULL ||
                  thpool_p->jobqueue_p->len == 0);//直到有工作
		  thpool_p->firstime = 1;

        }else {
            while(thpool_p->jobqueue_p->len == 0){//直到有工作 
                ATOMIC_LOAD(&thpool_p->jobqueue_p->len);
            }

            FTL_ATOMIC_ADD_FETCH(&thpool_p->num_threads_working, 1);//增加当前在工作的线程数目 
            ...
            if (g_newjob)
                g_newjob->function(g_newjob->arg);//执行真正的函数体

            FTL_ATOMIC_SUB_FETCH(&thpool_p->num_threads_working, 1);
        }
    }
    pthread_mutex_lock(&thpool_p->thcount_lock);
    thpool_p->num_threads_alive --;//线程死亡了
    pthread_mutex_unlock(&thpool_p->thcount_lock);
    return NULL;
}
...
/* Initialize queue */
static int jobqueue_init(thpool_* thpool_p){//初始化工作等待队列

    thpool_p->jobqueue_p = (struct jobqueue*)malloc(sizeof(struct jobqueue));
    if (thpool_p->jobqueue_p == NULL){
        return -1;
    }
    thpool_p->jobqueue_p->len = 0;//当前等待的工作队列数目
    thpool_p->jobqueue_p->front = NULL;//工作队列头部 
    thpool_p->jobqueue_p->rear  = NULL;//工作队列尾部 

    thpool_p->jobqueue_p->has_jobs = (struct bsem*)malloc(sizeof(struct bsem));//工作队列的信号量
    if (thpool_p->jobqueue_p->has_jobs == NULL){//
        return -1;
    }

    pthread_mutex_init(&(thpool_p->jobqueue_p->rwmutex), NULL);//工作队列的互斥锁
    bsem_init(thpool_p->jobqueue_p->has_jobs, 0);//初始化工作队列的信号量
...
static void bsem_init(bsem *bsem_p, int value) {
    ...
    pthread_mutex_init(&(bsem_p->mutex), NULL);//互斥锁 
    pthread_cond_init(&(bsem_p->cond), NULL);//条件变量
    bsem_p->v = value;//信号量的数目
}
