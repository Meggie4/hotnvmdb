...
namespace leveldb{
    ...
    void chunkTable::Add(const char* kvitem){
        ...
        Slice user_key = Slice(key_ptr1, key_length1 - 8);
        ...
        KeyFrequencies[user_key.ToString()]++;
    }
    ...
    std::map<std::string, uint32_t> chunkTable::SelectHotKeysWithMean(){
      ...
      uint64_t hot_keys_thresh = KeyFrequencies.size() * 1 / 2;
