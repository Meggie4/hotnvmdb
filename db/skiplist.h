...
#ifdef USE_OFFSETS
    Node(const Key& k, const Key& mem) : key_offset(reinterpret_cast<const Key>(mem - k)) { 
        //DEBUG_T("in NewNode\n");
    }
...
SkipList<Key,Comparator>::NewNode(const Key& key, int height, bool head_alloc) {
    ...
    if (return_special) {
        ...
        DEBUG_T("get offset_mem, end, offset_mem:%p\n", offset_mem);
    ...
    SkipList<Key,Comparator>::FindLessThan(const Key& key) const {
        ...
        void SkipList<Key,Comparator>::Insert(const Key& key, uint64_t s) {
            ...
            void SkipList<Key,Comparator>::Insert(const Key& key) {
                ...
                //DEBUG_T("SkipList insert, start, key:%p\n", key);
...
#endif
                /*if(arena_->nvmarena_)
                    DEBUG_T("SkipList insert, end,key:%p, alloc_rem:%zu\n", key, *alloc_rem);*/
            }
