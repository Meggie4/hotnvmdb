...
#ifdef USE_OFFSETS
    virtual const char *GetNodeKey(){return reinterpret_cast<char *>((intptr_t)iter_.node_ - (intptr_t)iter_.key_offset()); }
#else
...
void MemTable::Add(const char* kvitem){
    ...
    DEBUG_T("nvm immutable add, start, buf:%p\n", buf); 
#ifdef ENABLE_RECOVERY
    table_.Insert(buf);
#else
    table_.Insert(buf);
#endif
    DEBUG_T("nvm immutable add, end, buf:%p\n", buf); 
}
//////////////meggie
